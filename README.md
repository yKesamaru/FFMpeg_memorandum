...
GPUを使ったエンコード（NVENC）は **「速さ優先」**、CPUを使ったエンコード（libx264）は **「画質優先」** という違いがあります。

### **💡 1. CPUエンコード（libx264）は「丁寧」な圧縮をする**
**→ 時間をかけて高品質な圧縮をするので、同じビットレートでも画質が良い**

CPUは動画を圧縮する際、以下のように**細かく分析**しながらデータを削減します：
✅ **動きが少ない部分は強く圧縮する**（データを削減）
✅ **動きが激しい部分は画質を維持する**（重要な情報を残す）

つまり、CPUは「どの部分をどれくらい圧縮すれば画質を維持できるか」を **1フレームずつ計算** しながら、時間をかけてエンコードします。
...
**ただし、同じビットレートを指定しても、プリセットによってファイルサイズと処理時間が変わります。**

### **💡 `-preset` の仕組み（簡単なイメージ）**
プリセットは **「エンコードの丁寧さ」** を決める設定です。
...
```
もし字幕や外国語吹き替えを保持したい場合は以下を参考にしてください。
```bash
